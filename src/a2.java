...
public class a2 
{
    ...
    public static double[][] readCSVFile(String path) throws IOException 
    {
        ...
        while (dataSegment != null) 
        {
            ...
            {
                // If the value is "?", mark it as missing (NaN), otherwise convert it to a double
    ...
    /**
     * This method fills in the missing values (NaN) by calculating the mean for each feature (column).
     * If a value is missing in a row, it'll be replaced with the average of all of the other non-missing values in that column.
     */
    ...
    /**
     * This function calculates the Manhattan distance between two rows (or objects).
     * Manhattan distance is basically the sum of the absolute differences between the two rows.
     * If one of the values is missing, we'll make it further by adding 1 to the distance.
     */
     ...
     * For each row with missing data, it finds the most similar row (using Manhattan distance) that has no missing data.
     * The missing values in the row are replaced with values from the most similar row.
     */
    ...
    /**
     * This function calculates the Mean Absolute Error (MAE) between two datasets: the imputed one and the complete one.
     * It measures how far the imputed values are from the true values in the complete dataset.
     */
    public static double calculateMAE(double[][] imputedData, double[][] completeData) 
    {
        ...
        int count = 0;  // This will keep track of how many values we're comparing
               ...
               // Return the mean of the absolute differences (total error divided by number of comparisons)
